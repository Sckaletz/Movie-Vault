...
Fortsættelse af [uge 1](./uge-01-server-setup.md). Sidste uge stod fundamentet klar; i denne uge erstattede jeg test-placeholderen med gruppens **rigtige React-frontend** i Docker, og samlede derefter hele stacken (frontend + backend + database) bag mit eget domæne.

Siden kører på **varovejcontemporary.com** (Cloudflare, fra uge 1). Alle i gruppen har egen server + eget domæne, så vi hver især lærer at sætte hele kæden op selv.
...
- Skrevet `frontend/Dockerfile` som **multi-stage build**: stage 1 (`node:20-alpine`) bygger React-appen → `dist/`, stage 2 (`nginx:alpine`) serverer den på port 80. Tilføjet `frontend/.dockerignore`.
...
**Hele stacken med Docker Compose + reverse proxy**
...
- Lagt `.env`-filer på serveren. Db'en kører, men bruges ikke i koden endnu — bare for at have den klar.
